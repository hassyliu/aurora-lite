...
4. 在服务器列表点击“转发管理”，进入该服务器的规则页面。点击“添加转发”，选择 iptables 或 GOST，填写监听 IP/端口和目标地址/端口；规则自动归属当前服务器。
5. 新建规则保存后自动下发，在弹窗直接查看执行进度及结果；失败后可修复原因并点击“重新下发”。
6. 运行中的规则可直接编辑，保存后自动清理旧配置并启动新配置。手动“终止”后编辑仍保持停止，可点击“启用”恢复。删除会先清理远程服务，成功后删除本地记录。
...
编辑规则会回显上次选择的落地；手动调整目标后解除关联。已有转发规则保存目标地址的独立副本；修改或删除落地条目不会自动改动已保存的规则。完整数据库备份与配置清单导出包含落地地址。

### 直接查看检查结果

服务器详情弹窗和转发管理页的“检查连接”会直接展示检查中、成功或失败，以及时间、SSH 检查耗时、组件状态。这里显示的是检查耗时，不是网络 Ping 延迟。
