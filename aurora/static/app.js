...
const state = {user:null, page:'overview', serverId:null, servers:[], rules:[], tasks:[], destinations:[], overview:null, search:'', method:'all', refreshing:false, sending:{}, inspectionId:null, reordering:false, orderRevision:0, drag:null, serverSearch:'', serverSort:'manual', viewer:null};
const titles = {
...
function renderInspection(){
  ...
  if(pending){const task=state.tasks.find(t=>t.rule_id===rule.id&&t.action==='diagnose'&&['pending','running'].includes(t.state));if(task?.log)$('#inspection-content').insertAdjacentHTML('beforeend',`<pre class="check-live-log">${esc(task.log)}</pre>`);}
...
function filteredServers(){
  ...
  const servers=state.servers.filter(s=>[s.name,s.host,s.notes].some(value=>String(value||'').toLowerCase().includes(q)));
  if(state.serverSort==='name')servers.sort((a,b)=>a.name.localeCompare(b.name,'zh-CN'));
  if(state.serverSort==='host')servers.sort((a,b)=>a.host.localeCompare(b.host,undefined,{numeric:true}));
  if(state.serverSort==='rules')servers.sort((a,b)=>b.rule_count-a.rule_count);
  return servers;
}
function serverListContent(){
  const servers=filteredServers(),manual=state.serverSort==='manual'&&!state.serverSearch.trim();
  if(!servers.length)return empty('没有匹配的服务器','试试服务器名称、IP 或备注中的关键词。','','','server');
  return `<div class="server-list-head"><span>排序</span><span>服务器</span><span>SSH 地址</span><span>连接状态</span><span>规则</span><span>操作</span></div><div class="server-list" role="list" aria-label="服务器列表">${servers.map(server=>{
    const index=state.servers.findIndex(s=>s.id===server.id),disabled=!manual||state.reordering;
    return `<article class="server-row" role="listitem" data-server="${server.id}"><div class="server-sort-actions"><button class="drag-handle" data-drag-server="${server.id}" aria-label="拖动排序 ${esc(server.name)}" ${disabled?'disabled':''}>${icon('grip')}</button><div><button class="sort-step" data-move-server="${server.id}" data-offset="-1" aria-label="上移 ${esc(server.name)}" ${disabled||index===0?'disabled':''}>${icon('up')}</button><button class="sort-step" data-move-server="${server.id}" data-offset="1" aria-label="下移 ${esc(server.name)}" ${disabled||index===state.servers.length-1?'disabled':''}>${icon('down')}</button></div></div><a class="server-row-name" href="${serverRulesLink(server.id)}" title="${esc(server.name+(server.notes?' · '+server.notes:''))}">${esc(server.name)}</a><span class="mono server-row-address" title="${esc(endpoint(server.host,server.port))}">${esc(endpoint(server.host,server.port))}</span><span>${checking(server.id,'check')?badge('pending'):badge(server.state)}</span><a class="rule-link" href="${serverRulesLink(server.id)}">${server.rule_count} 条</a><div class="server-row-actions"><a class="btn primary tiny" href="${serverRulesLink(server.id)}">转发管理</a>${connectionButton(server)}<button class="btn secondary tiny" data-server-detail="${server.id}">详情</button><button class="btn secondary tiny" data-edit-server="${server.id}">编辑</button></div></article>`;
  }).join('')}</div>`;
}
function serversPage(){
  if(!state.servers.length)return `<div class="panel">${empty('连接你的第一台服务器','使用 SSH 密码或私钥连接 Debian / Ubuntu 中转机。','添加服务器')}</div>`;
  return `<div class="panel"><div class="toolbar"><div class="search-field">${icon('search')}<input id="server-search" value="${esc(state.serverSearch)}" placeholder="搜索服务器名称、IP 或备注" aria-label="搜索服务器"></div><select id="server-sort" aria-label="服务器排序">${option('manual','自定义顺序',state.serverSort)}${option('name','按名称排序',state.serverSort)}${option('host','按地址排序',state.serverSort)}${option('rules','按规则数量排序',state.serverSort)}</select><span class="count">共 ${state.servers.length} 台服务器</span></div><div class="server-list-scroll"><div id="server-list-content">${serverListContent()}</div></div><p class="destination-note">${state.reordering?'正在保存排序…':'自定义顺序下可拖动手柄或点击箭头排序，顺序自动保存。搜索时保留当前顺序。'}</p></div>`;
}
...
function ruleListContent(){const rules=serverRules(),filtered=filteredRules();return filtered.length?ruleTable(filtered):empty(rules.length?'没有匹配的规则':'这台服务器还没有转发规则',rules.length?'试试其他关键词或筛选条件。':'填写监听端口和目标地址，保存后自动下发并启用。',rules.length?'':'添加转发','rule','route');}
...
function render(){
  ...
  if(state.page==='servers'&&focused?.id==='server-search'){$('#server-list-content').innerHTML=serverListContent();return;}
...
async function refresh(){
  if(!state.user||state.refreshing)return;state.refreshing=true;const orderRevision=state.orderRevision;
  try{const [overview,servers,rules,tasks,destinations]=await Promise.all(['/overview','/servers','/rules','/tasks','/destinations'].map(p=>api(p)));Object.assign(state,{overview,rules,tasks});if(!state.drag&&!state.reordering&&orderRevision===state.orderRevision)Object.assign(state,{servers,destinations});$('#load-error').hidden=true;$('#connection-state').innerHTML='<span class="status-dot"></span> 面板已连接';render();}
...
function openRule(id){
  ...
  if(serverBusy(server.id)){toast('这台服务器正在执行任务，请完成后再编辑',true);return;}
  editorType='rule';editorId=id||null;$('#editor-title').textContent=id?'编辑转发规则':'添加转发';
  $('#editor-body').innerHTML=`<div class="form-grid"><label class="full">规则名称<input name="name" value="${esc(r.name)}" placeholder="例如：游戏服务器中转" required maxlength="80"></label><label class="full">中转服务器<select name="server_id" disabled>${option(server.id,server.name+' · '+server.host,server.id)}</select><span class="form-hint">此规则属于当前服务器。</span></label><label>中转方式<select name="method">${option('iptables','iptables · 内核转发',r.method)}${option('gost','GOST · 应用转发',r.method)}</select></label><label>传输协议<select name="protocol">${option('tcp','TCP',r.protocol)}${option('udp','UDP',r.protocol)}${option('both','TCP + UDP',r.protocol)}</select></label><label>监听 IP<input name="listen_ip" value="${esc(r.listen_ip)}" required></label><label>监听端口<input name="listen_port" value="${r.listen_port||''}" placeholder="例如 10080" type="number" min="1" max="65535" required></label><label class="full">快速选择落地<select id="destination-picker" name="destination_id"><option value="">手动填写目标</option>${state.destinations.map(d=>option(d.id,d.name+' · '+endpoint(d.host,d.port),r.destination_id)).join('')}</select><span class="form-hint">${state.destinations.length?'选择后填入目标地址与端口，仍可手动调整。':'可先到“落地管理”保存常用地址。'}</span></label><label>目标 IP / 域名<input name="target_host" value="${esc(r.target_host)}" placeholder="目标服务器地址" required></label><label>目标端口<input name="target_port" value="${r.target_port||''}" placeholder="例如 443" type="number" min="1" max="65535" required></label><div class="form-hint full">0.0.0.0 监听全部 IPv4 地址，:: 监听 IPv6。iptables 目标需为同一 IP 版本的固定地址；域名或跨 IPv4/IPv6 使用 GOST。${id&&r.desired==='stopped'?'此规则已手动停止，保存后保持停止，点击“启用”恢复。':'保存后自动下发。修改运行中的规则会短暂重启该条转发。'}</div><label class="full">备注<textarea name="notes" maxlength="1000" placeholder="这条规则的用途">${esc(r.notes)}</textarea></label></div>`;
  $('#editor-error').textContent='';$('#editor').showModal();
}
function view(title,content){
  state.viewer=null;$('#viewer-title').textContent=title;$('#viewer-content').textContent=content;
  $('#viewer-content').hidden=false;$('#viewer-details').hidden=true;$('#viewer-status').textContent='';
  if(!$('#viewer').open)$('#viewer').showModal();
}
function openLiveViewer(type,id){
  view(type==='logs'?'服务日志':type==='task'?'执行日志':'服务器详情','正在读取…');
  state.viewer={type,id,loading:false,lastRefresh:0};renderViewer();refreshViewer();
}
function setViewerText(text){
  const node=$('#viewer-content'),follow=node.scrollHeight-node.scrollTop-node.clientHeight<45;
  if(node.textContent!==text){node.textContent=text;if(follow)node.scrollTop=node.scrollHeight;}
}
function renderViewer(){
  const viewer=state.viewer;if(!viewer||!$('#viewer').open)return;
  if(viewer.type==='server'){
    const s=state.servers.find(s=>s.id===viewer.id);
    if(!s){$('#viewer').close();return;}
    $('#viewer-title').textContent=s.name+' · 服务器详情';$('#viewer-content').hidden=true;$('#viewer-details').hidden=false;
    $('#viewer-status').textContent='连接状态与任务结果自动刷新';
    $('#viewer-details').innerHTML=`<div class="server-detail-grid"><span>SSH 地址</span><strong class="mono">${esc(endpoint(s.host,s.port))}</strong><span>SSH 账号</span><strong>${esc(s.username)} · ${s.auth_type==='key'?'私钥':'密码'}</strong><span>主机指纹</span><span class="mono">${esc(s.fingerprint)}</span><span>备注</span><span>${esc(s.notes||'—')}</span></div>${connectionFeedback(s)}<div class="server-detail-actions">${connectionButton(s)}<button class="btn secondary tiny" data-server-action="prepare" data-id="${s.id}" ${serverBusy(s.id)?'disabled':''}>安装中转组件</button><button class="btn secondary tiny" data-delete-server="${s.id}">删除服务器</button></div>`;
  }else if(viewer.type==='task'){
    const task=viewer.task||state.tasks.find(t=>t.id===viewer.id);if(!task)return;
    $('#viewer-title').textContent=(actions[task.action]||task.action)+' · '+(labels[task.state]||task.state);
    $('#viewer-status').textContent=['pending','running'].includes(task.state)?'每秒自动刷新执行进度':'任务已结束 · '+date(task.finished);
    setViewerText(task.log||'任务已提交，等待执行…');
  }
}
async function refreshViewer(){
  const viewer=state.viewer;if(!viewer||!$('#viewer').open||viewer.loading||viewer.type==='server'||!state.user)return;
  if(Date.now()-viewer.lastRefresh<(viewer.type==='logs'?3000:1000))return;
  if(viewer.type==='task'&&viewer.task&&!['pending','running'].includes(viewer.task.state))return;
  viewer.loading=true;viewer.lastRefresh=Date.now();
  try{
    if(viewer.type==='task'){
      const task=await api('/tasks/'+viewer.id);
      if(state.viewer!==viewer)return;viewer.task=task;renderViewer();
    }else{
      const rule=state.rules.find(r=>r.id===viewer.id);
      if(!rule){$('#viewer').close();return;}
      $('#viewer-title').textContent=rule.name+' · 服务日志';
      if(serverBusy(rule.server_id)){
        const task=state.tasks.find(t=>t.server_id===rule.server_id&&['pending','running'].includes(t.state));
        $('#viewer-status').textContent='服务器正在执行任务，完成后自动继续读取服务日志';
        if(task)setViewerText(task.log||'任务执行中…');return;
      }
      const result=await api('/rules/'+viewer.id+'/logs');
      if(state.viewer!==viewer)return;
      setViewerText(result.log||'当前没有服务日志');$('#viewer-status').textContent='每 3 秒自动刷新 · '+date(result.updated_at);
    }
  }catch(error){if(state.viewer===viewer)$('#viewer-status').textContent=error.message+' · 将自动重试';}
  finally{viewer.loading=false;}
}
$('#viewer').addEventListener('close',()=>{state.viewer=null;});
...
async function queue(path){
  ...
  try{
    ...
    if(result.task_id&&action!=='diagnose')openLiveViewer('task',result.task_id);
    toast(action==='diagnose'?'正在检测当前转发规则':'操作已提交，进度将在弹窗中更新');
...
async function handleClick(event){
  ...
  if(d.serverDetail){openLiveViewer('server',d.serverDetail);return;}
  if(d.moveServer){await moveServer(d.moveServer,Number(d.offset));return;}
  ...
  if(d.taskLog){openLiveViewer('task',d.taskLog);return;}
  ...
  if(d.ruleAction==='logs'){openLiveViewer('logs',d.id);return;}
...
async function saveDestinationOrder(ids,kind='destination'){
  if(state.reordering)return;
  const key=kind==='server'?'servers':'destinations';
  const items=new Map(state[key].map(d=>[d.id,d]));
  if(ids.length!==items.size||ids.some(id=>!items.has(id)))return;
  state[key]=ids.map((id,index)=>({...items.get(id),position:index}));state.reordering=true;state.orderRevision++;render();
  try{await api('/'+key+'/order',{method:'PUT',body:JSON.stringify({ids})});toast((kind==='server'?'服务器':'落地')+'排序已保存');}
  catch(error){toast(error.message,true);try{state[key]=await api('/'+key);}catch{toast('排序未确认，请刷新页面',true);}}
  finally{state.reordering=false;state.orderRevision++;render();}
}
...
async function moveServer(id,offset){
  if(state.reordering||state.drag||state.serverSort!=='manual'||state.serverSearch.trim())return;
  const ids=state.servers.map(s=>s.id),index=ids.indexOf(id),next=index+offset;
  if(index<0||next<0||next>=ids.length)return;
  [ids[index],ids[next]]=[ids[next],ids[index]];await saveDestinationOrder(ids,'server');
}
function positionDestinationDrag(){
  const drag=state.drag;if(!drag||!drag.moved)return;
  const rows=[...drag.list.querySelectorAll(drag.kind==='server'?'[data-server]':'[data-destination]')].filter(row=>row!==drag.row);
...
document.addEventListener('pointerdown',event=>{
  const handle=event.target.closest('[data-drag-destination],[data-drag-server]');
  if(!handle||handle.disabled||state.reordering||state.drag||event.button!==0)return;
  const kind=handle.dataset.dragServer?'server':'destination',row=handle.closest(kind==='server'?'[data-server]':'[data-destination]');event.preventDefault();handle.focus();
  state.drag={kind,handle,row,list:row.parentElement,pointerId:event.pointerId,startY:event.clientY,clientY:event.clientY,moved:false};
...
document.addEventListener('pointerup',event=>{
  const drag=state.drag;if(!drag||event.pointerId!==drag.pointerId)return;
  const ids=[...drag.list.querySelectorAll(drag.kind==='server'?'[data-server]':'[data-destination]')].map(row=>drag.kind==='server'?row.dataset.server:row.dataset.destination);
  const changed=ids.join()!==state[drag.kind==='server'?'servers':'destinations'].map(d=>d.id).join();cancelDestinationDrag();
  if(changed)saveDestinationOrder(ids,drag.kind);else render();
});
...
document.addEventListener('keydown',event=>{
  ...
  const handle=event.target.closest('[data-drag-destination],[data-drag-server]');
  if(handle&&['ArrowUp','ArrowDown'].includes(event.key)){event.preventDefault();if(handle.dataset.dragServer)moveServer(handle.dataset.dragServer,event.key==='ArrowUp'?-1:1);else moveDestination(handle.dataset.dragDestination,event.key==='ArrowUp'?-1:1);}
});
document.addEventListener('click',event=>handleClick(event).catch(error=>toast(error.message,true)));
document.addEventListener('input',event=>{if(event.target.id==='server-search'){state.serverSearch=event.target.value;$('#server-list-content').innerHTML=serverListContent();}if(event.target.id==='rule-search'){state.search=event.target.value;$('#rule-list').innerHTML=ruleListContent();}if(['target_host','target_port'].includes(event.target.name)&&$('#destination-picker'))$('#destination-picker').value='';});
document.addEventListener('change',event=>{
  if(event.target.id==='server-sort'){state.serverSort=event.target.value;render();}
...
$('#editor-form').addEventListener('submit',async event=>{
  ...
  if(editorType==='server'||editorType==='destination')data.port=Number(data.port);else{data.listen_port=Number(data.listen_port);data.target_port=Number(data.target_port);data.server_id=form.elements.server_id.value;data.destination_id=form.elements.destination_id.value||null;}
  try{const result=await api(`/${editorType==='server'?'servers':editorType==='destination'?'destinations':'rules'}${editorId?'/'+editorId:''}`,{method:editorId?'PUT':'POST',body:JSON.stringify(data)});$('#editor').close();toast(editorType==='rule'?(result.task_id?'已保存，正在自动下发':'已保存，规则保持停止'):editorType==='destination'?'落地已保存':'服务器已保存');await refresh();if(result.task_id)openLiveViewer('task',result.task_id);}catch(error){$('#editor-error').textContent=error.message;}finally{button.disabled=false;}
});
